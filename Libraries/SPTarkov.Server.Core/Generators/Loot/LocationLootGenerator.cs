...
/// <summary>
/// Location loot generation runs in <c>rust/spt-native/src/loot/location_loot_generator.rs</c> by
/// default; the entry points project the live database and config into the native payload and
/// replay the log lines the native side collected. The full 4.1.2 C# implementation is retained
/// below as the legacy path — it is the frozen mod contract (constructor, protected members and
/// DTOs are apicompat-gated against the 4.1.2 baseline) and runs instead of the native path when a
/// Harmony patch on any protected member is detected or LocationConfig.ForceLegacyLootGeneration
/// is set, so mod hooks fire with genuine baseline semantics.
/// </summary>
[Injectable]
...
)
{
    ...
    private List<SpawnpointTemplate> GenerateDynamicLootLegacy(
        LooseLoot dynamicLootDist,
        Dictionary<string, IEnumerable<StaticAmmoDetails>> staticAmmoDist,
        string locationName
    )
